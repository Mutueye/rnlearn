...
class CNTextInput extends Component {
  ...
  findContentIndex(content, cursorPosition) {
    ...
    if(findIndx == -1) findIndx = content.length - 1;
  ...
  onSelectionChange = (event) => {
    if (this.justToolAdded) {
      ...
      if(upComingStype !== null) {
        if(upComingStype.sel.start === this.prevSelection.start && upComingStype.sel.end === this.prevSelection.end) {
      ...
      if(this.avoidSelectionChangeOnFocus) this.justToolAdded = true;
      this.avoidSelectionChangeOnFocus = false;
      //if(this.avoidAndroidJump == true) this.avoidSelectionChangeOnFocus = true;
  ...
  handleChangeText = (text) => {
    ...
    if(content[foundIndex].readOnly === true && textDiff >= 0) {
      if(content[foundIndex].text.length === foundItemNo) {
        if(content.length > foundIndex + 1 && !(content[foundIndex + 1].readOnly === true)) {
          foundIndex +=1;
          foundItemNo == 0; 
        } else {  
          if(this.upComingStype && this.upComingStype.sel.end === cursorPosition) {
    ...
    if(this.upComingStype !== null && startWithReadonly === false && this.upComingStype.sel.end === cursorPosition && textDiff >= 0) {
    ...
    if(textDiff >= 0) {
      ...
      const addedText = text.substring(cursorPosition, cursorPosition + textDiff)
  ...
  updateNewLine(content, index, itemNo) {
    ...
    if(itemNo === 0) {
      ...
      if(newContent.length > index + 1) {
        ...
        if(isPrevList === true) newContent = this.changeToTagIn(newContent, prevTag , index + 1, true);
      }
    } else {
      beforeContent = {
        id: foundElement.id,
  ...
  addToUpComming(toolType) {
    if(this.upComingStype) {
      ...
      let newUpStype = this.upComingStype ? (indexOfUpToolType != -1 ? update(this.upComingStype.stype, {$splice: [[indexOfUpToolType, 1]]}) : update(this.upComingStype.stype, { $push: [toolType] })) : [toolType];
  ...
  applyStyle(toolType) {   
    ...
    for (var i = 0; i < content.length; i++) {
      ...
      let newStype =(indexOfToolType != -1) ? update(stype, {$splice: [[indexOfToolType, 1]]}) : update(stype, { $push: [toolType] });
