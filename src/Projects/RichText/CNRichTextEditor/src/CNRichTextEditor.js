...
class CNRichTextEditor extends Component {
  ...
  removeImage =(index) => {
    ...
    if(content.component === 'image') {
      ...
      if(this.props.onRemoveImage) this.props.onRemoveImage({ id :removedId, url:removedUrl });
    }
  }
  ...
  onContentChanged = (items, index) => {
    ...
    input.content = items;
    this.props.onValueChanged( update(this.props.value, { [index]: {$set : input}}) );
  }
  ...
  renderInput(input, index, isLast) {
    ...
    return (
      <CNTextInput 
        ...
        styleList={styles}
        style={isLast === true ? { borderWidth: 0, flexGrow: 1 } : { borderWidth: 0, flexGrow: 0 }}
      />
    );
  }
    ...
    return (
      <View 
        ...
        <TouchableWithoutFeedback onPress={() => this.onImageClicked(index)}>
          <Image
            style={{width: image.size.width, height: image.size.height, opacity: this.state.imageHighLightedInex === index ? .8  : 1}}
  ...
  render() {
    ...
    return (
      <ScrollView 
        contentContainerStyle={{ flexGrow: 1, alignContent: 'flex-start', justifyContent: 'flex-start'}} 
        onLayout={this.onLayout} 
        style={[style]}>
        {
          _.map(value, (item,index) => {
            if(item.component === 'text') {
              return this.renderInput(item , index, index === value.length - 1)
