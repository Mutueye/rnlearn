...
export function convertToHtmlString(contents, styleList = null) {
  ...
  for (let i = 0; i < contents.length; i++) {
    ...
    if(input.component === 'text') {
      ...
      if(input.imageId) element.setAttribute('data-id',input.imageId);
      myDoc.documentElement.appendChild(element);
    }
  }
      ...
      if(tag == 'ul' || tag == 'ol') {
        for (let k = 0; k < element.childNodes.length; k++) {
          ...
          for (let j = 0; j < node.childNodes.length; j++) {
            ...
            item.content.push(xmlNodeToItem(child, tag,false));
          }
        }
      } else {
        for (let j = 0; j < element.childNodes.length; j++) {
          ...
          if(firstLine) childItem.NewLine = true;
...
function xmlNodeToItem(child, tag, newLine, styleList = null) {
  ...
  let stype = [];
  if(isBold) stype.push('bold');
  if(isItalic) stype.push('italic');
  if(isUnderLine) stype.push('underline');
  if(isOverline) stype.push('lineThrough');
  if(isBlue) stype.push('blue');
  if(isGreen) stype.push('green');
  if(isRed) stype.push('red');
  if(isBlueMarker) stype.push('blue_hl');
  if(isOrangeMarker) stype.push('orange_hl');
  if(isYellowMarker) stype.push('yellow_hl');
  if(isGreenMarker) stype.push('green_hl');
  if(isPinkMarker) stype.push('pink_hl');
  if(isPurpleMarker) stype.push('purple_hl');
